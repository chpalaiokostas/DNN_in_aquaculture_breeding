...
# Example of fitting a local convolutional neural network (LCNN)
model_lcnn =keras.models.Sequential([
    keras.layers.LocallyConnected1D(12,12,strides=10,input_shape=(X_train.shape[1],1)),
    keras.layers.LocallyConnected1D(12,12,strides=10),
    keras.layers.LocallyConnected1D(12,12,strides=10),
...
model_lcnn.compile(loss='mean_squared_error',optimizer=optimizer)
history_lcnn = model_lcnn.fit(X_train, y_train, epochs=100,
                   validation_data=(X_valid,y_valid), callbacks=[early_stopping_cb])
mse_test = model_lcnn.evaluate(X_test, y_test)
print(f"The mean squared error is: {mse_test}")
y_pred = model_lcnn.predict(X_test)
